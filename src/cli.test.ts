...
test('reduce subcommnad', async (t: ExecutionContext) => {
  const cmd = `echo '[{"val":1},{"val":2},{"val":3}]' | jqf reduce '(prev, {val}) => prev + val' '0'`
  const { stdout } = await exec(cmd)
  t.is(stdout, '6')
})

test('reduceRight subcommnad', async (t: ExecutionContext) => {
  const cmd = `echo '["a","b","c"]' | jqf reduceRight '(prev, str) => prev + str' '"word: "'`
  const { stdout } = await exec(cmd)
  t.is(stdout, '"word: cba"')
})

test('reduce subcommnad fails with invalid JSON', async (t: ExecutionContext) => {
